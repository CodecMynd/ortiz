...
if ($super == 1 or $verTablaAutoProceSurtPzsAdic == 1) {
	...
	MAX(R.id_regSolpzadicional) AS id_regSolpzadicional, MAX(R.folioPzAdicional) AS folioPzAdicional, R.id_regSolpzadicional AS id_regSolPzs,
    ...
    AP.id_autorizadoPzsAdic, MAX(AP.folio_autorizPzsAdic) AS folio_autorizPzsAdic, AP.cronoAutorizadoPzAdic,
    ...
    INNER JOIN autoprocesurtpzsadic APS ON P.id_proyecto = APS.id_proyecto
	WHERE P.estadoProyectoEliminado = 1 AND P.proyectoActivo = 1 AND P.autoProceSurtPzAdic = 1 AND APS.borrado = 0 GROUP BY P.id_proyecto ORDER BY R.id_regSolpzadicional DESC";
} else {
...
while ($row = $resultado->fetch_assoc()) {
	...
	if ($super == 1 or $enviarPzsEntregadasPzsAdic == 1) {
		...
		$outputBtns2 = "<a class='btn btn-outline-danger' id='enviarPzsEntregadasPzsAdic' data-toggle='tooltip' data-placement='left' title='Sin Permiso'><i class='fa-solid fa-paper-plane'></i></a>";
	}

	// 4.2.4.3 Ver Generales Proceso de Surtido de Piezas: Piezas Adicionales
	if ($super == 1 or $verGralProceSurtPzsAdic == 1) {
		$outputBtns3 = "<a href='javascript:void(0)' class='btn btn-secondary' onclick='mostarDetalles3(\"" . $row['id_proyecto'] . "\")'><i class='fa-solid fa-eye'></i></a>";
	} else {
