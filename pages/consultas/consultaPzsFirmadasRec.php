...
if ($super == 1 or $verTablaPzsFirmadasRec == 1) {
	...
	WHERE P.estadoProyectoEliminado = 1 AND P.pzsFirmadaRec = 1 AND PF.borrado = 0 GROUP BY P.id_proyecto ORDER BY RC.id_regCompraInicial DESC";
} else {
...
while ($row = $resultado->fetch_assoc()) {
	...
	// 4.1.7.1 Regresar de Piezas Firmadas de Recibido a Piezas Entregadas
	if ($super == 1 or $regresarPzsEntregadas == 1) {
		$outputBtns16 = "<a href='#' class='btn btn-secondary' onclick='abrirModal13(\"" . $idP . "\",\"" . $nP . "\",\"" . $row['id_recPzsDanadas'] . "\",\"" . $id_solPzsDanadas . "\",\"" . $id_regCompraInicial . "\",\"" . $id_autorizado . "\", \"" . $id_pzsEntregadas . "\",\"" . $row['id_pzsFirmadasRec'] . "\")'><i class='fa-solid fa-reply'></i></a>";
	} else {
		$outputBtns16 = "<a class='btn btn-outline-danger' id='regresarPzsEntregadas' data-toggle='tooltip' data-placement='left' title='Sin Permiso'><i class='fa-solid fa-reply'></i></a>";
	}

	// 4.1.7.2 Enviar de Piezas Firmadas de Recibido a Supervisión de Surtido de Piezas
	if ($super == 1 or $enviarSuperSurtPzs == 1) {
		$outputBtns17 = "<a href='#' class='btn btn-secondary' onclick='abrirModal14(\"" . $idP . "\",\"" . $nP . "\",\"" . $row['id_recPzsDanadas'] . "\",\"" . $id_solPzsDanadas . "\",\"" . $id_regCompraInicial . "\",\"" . $id_autorizado . "\", \"" . $id_pzsEntregadas . "\",\"" . $row['id_pzsFirmadasRec'] . "\")'><i class='fa-solid fa-paper-plane'></i></a>";
	} else {
		$outputBtns17 = "<a class='btn btn-outline-danger' id='enviarSuperSurtPzs ' data-toggle='tooltip' data-placement='left' title='Sin Permiso'><i class='fa-solid fa-paper-plane'></i></a>";
	}

	// 4.1.7.3 Ver Generales Piezas Firmadas de Recibido
	if ($super == 1 or $verGralPzsFirmadasRec == 1) {
		$outputBtns18 = "<a href='javascript:void(0)' class='btn btn-secondary' onclick='mostarDetalles5(\"" . $row['id_proyecto'] . "\",\"" . $row['id_preAutorizado'] . "\",\"" . $row['id_pzsEntregadas'] . "\",\"" . $row['id_pzsFirmadasRec'] . "\")'><i class='fa-solid fa-eye'></i></a>";
	} else {
		$outputBtns18 = "<a class='btn btn-outline-danger' id='verGralPzsFirmadasRec' data-toggle='tooltip'  title='Sin Permiso'><i class='fa-solid fa-eye'></i></a>";
	}
	...
	$datos[] = array(
		...
		"17" => "<strong>{$row['cronoPzsFirmadasRec']}</strong>",
		"18" => "<strong>{$row['cronoAutorizado']}</strong>",
		"19" => "<strong>{$row['cronoPreAuto']}</strong>",
						...
						<button type='button' class='btn btn-secondary dropdown-toggle' data-toggle='dropdown'><i class='fas fa-cog'></i><span data-toogle='tooltip' title='Botónes de administración  tabla Recepción de Piezas Dañadas'> Acciones</span></button>
							<ul class='dropdown-menu text-center' style='columns:2; min-width:2em;'>
								<li class='dropdown-item'>
									<span data-toggle='tooltip' title='4.1.7.2 Enviar de Piezas Firmadas de Recibido a Supervisión de Surtido de Piezas'>
										" . $outputBtns16 . "
									</span>
								</li>
								<li class='dropdown-item'>
								...
								<li class='dropdown-item'>
									<span data-toggle='tooltip' title='4.1.7.3 Ver Generales Piezas Firmadas de Recibido'>
										" . $outputBtns18 . "
									</span>
								</li>
								<li class='dropdown-item'>
