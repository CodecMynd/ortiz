...
while ($row = $resultado->fetch_assoc()) {
	...
	if($supervision == 0){
		$outputBtns5 = "<a class='btn btn-outline-danger' id='enviarPreAutoPzsAdic' data-toggle='tooltip'  title='Aún no ha sido Supervisado'><i class='fa-solid fa-paper-plane'></i></a>";
	} else if ($super == 1 AND $supervision == 1) {
		$outputBtns5 = "<a href='#' class='btn btn-secondary' onclick='abrirModal4(\"" . $idP . "\",\"" . $nP . "\",\"" . $row['id_proyExtra'] . "\")'><i class='fa-solid fa-paper-plane'></i></a>";
	} else if ($enviarProyUtil == 1 AND $supervision == 1) {
		$outputBtns5 = "<a href='#' class='btn btn-secondary' onclick='abrirModal4(\"" . $idP . "\",\"" . $nP . "\",\"" . $row['id_proyExtra'] . "\")'><i class='fa-solid fa-paper-plane'></i></a>";
	} else {
		$outputBtns5 = "<a class='btn btn-outline-danger' id='enviarPreAutoPzsAdic' data-toggle='tooltip'  title='Sin Permiso'><i class='fa-solid fa-paper-plane'></i></a>";
	}
